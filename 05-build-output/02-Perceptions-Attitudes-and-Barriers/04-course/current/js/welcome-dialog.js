...
(function() {
  ...
  try {
    var progressRaw = localStorage.getItem('guide02_progress');
    if (progressRaw) {
  ...
  function resetProgress() {
    localStorage.removeItem('guide02_progress');
    localStorage.removeItem('courseVisitedSlides');
    localStorage.removeItem('courseSubmissions');
    localStorage.removeItem('guide02_map_responses');
  ...
  if (savedUser && savedUser.name && savedSlide) {
    ...
    card.innerHTML =
      ...
      '<p class="welcome-sub">Guide 02: Perceptions, Attitudes, and Barriers</p>' +
