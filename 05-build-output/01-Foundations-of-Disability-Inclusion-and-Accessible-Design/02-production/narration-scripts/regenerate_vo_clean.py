...
API_KEY = os.environ.get('ELEVENLABS_API_KEY', 'sk_aa9e6c75d5c7189d4643e6ecc34edf039be3f962599916c8')
VOICE_ID = "FVQMzxJGPUBtfz1Azdoy"
OUTPUT_DIR = os.path.dirname(os.path.abspath(__file__))
MOCKUP_VO_DIR = os.path.join(os.path.dirname(os.path.abspath(__file__)), '..', '..', '..', '02-branding-and-style', 'mockups', 'vo')

def clean_for_tts(text):
    """Clean text for natural TTS output"""
    # Replace em dashes used as parenthetical with commas
    # Pattern: "word — phrase — word" (parenthetical)
    text = re.sub(r'\s—\s([^—]+)\s—\s', r', \1, ', text)
    # Replace remaining em dashes with a period or comma for pause
    text = re.sub(r'\s—\s', '. ', text)
    # "AODA" should be spelled out
...
# All 22 narration scripts (raw, with em dashes)
scripts_raw = {
...
for screen_id, raw_text in scripts_raw.items():
    ...
    if resp.status_code == 200:
        ...
        # Also copy to mockup vo/ folder
        mockup_path = os.path.join(MOCKUP_VO_DIR, filename)
...
print("\nDone! All voiceovers regenerated with clean TTS text.")
