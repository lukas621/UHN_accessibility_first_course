...
def setup_styles(doc: Document) -> None:
    """Configure document styles — matches UHN Primary_StandardWord template.
    Uses Arial throughout, UHN brand colours from the official template."""
    # Normal — Arial 11pt (matching UHN template)
    ...
    # Heading 1 — Arial 16pt Bold, UHN Navy #192858 (from template)
    ...
    # Heading 2 — Arial 14pt Bold (from template)
    h2 = doc.styles['Heading 2']
    h2.font.name = 'Arial'
    h2.font.size = Pt(14)
    h2.font.color.rgb = UHN_NAVY
    h2.font.bold = True
    h2.paragraph_format.space_before = Pt(18)
    h2.paragraph_format.space_after = Pt(8)

    # Heading 3 — Arial 12pt, UHN Cobalt #255CAA (from template)
    h3 = doc.styles['Heading 3']
    h3.font.name = 'Arial'
    h3.font.size = Pt(12)
    h3.font.color.rgb = UHN_COBALT
    h3.font.bold = False
    h3.paragraph_format.space_before = Pt(12)
    h3.paragraph_format.space_after = Pt(6)

    # Heading 4 — Arial 12pt Bold (from template)
...
def add_header_footer(doc: Document, title: str, guide_num: int) -> None:
    """Add UHN-branded header and footer. If using UHN template, headers/footers
    are already present — we add/update the text content."""
    section = doc.sections[0]

    # Header — add logo + series title
    header = section.header
    header.is_linked_to_previous = False
    # Clear existing header content if from template
    for p in header.paragraphs:
        p.clear()
    header_para = header.paragraphs[0] if header.paragraphs else header.add_paragraph()

    # Add UHN logo if available
    if UHN_LOGO.exists():
        try:
            run = header_para.add_run()
            run.add_picture(str(UHN_LOGO), width=Cm(3))
        except Exception:
            pass  # Skip logo if it fails to load

    # Add series text to header
    run = header_para.add_run(f"    Accessibility First Series | Guide {guide_num:02d}")
    run.font.name = 'Arial'
    run.font.size = Pt(8)
    run.font.color.rgb = RGBColor(0x7F, 0x7F, 0x7F)

    # Footer — UHN confidential notice
...
def convert_md_to_docx(md_path: Path, docx_path: Path, guide_num: int) -> None:
    """Convert a markdown file to a UHN-branded Word document.
    Uses the official UHN Primary_StandardWord template as base if available."""
    with open(md_path, "r", encoding="utf-8") as f:
        content = f.read()

    # Use UHN template as base if available (inherits headers, footers, logo, styles)
    if UHN_TEMPLATE.exists():
        doc = Document(str(UHN_TEMPLATE))
        # Clear template body content but keep headers/footers/styles
        for p in doc.paragraphs:
            p.clear()
        # Remove empty paragraphs left from clearing
