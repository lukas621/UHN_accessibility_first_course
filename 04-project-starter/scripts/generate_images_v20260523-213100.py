...
- Do NOT include any logos, brand names, or text on buildings/signage — real logos will be overlaid in Storyline
- Do NOT generate any text, words, or letters in the image — text will be added separately
"""
...
def post_process_image(image_path: Path, target_width: int = 1920, target_height: int = 1080) -> None:
    """Upscale to Storyline 360 resolution and sharpen."""
    try:
        ...
        pass  # Pillow not installed — skip post-processing
...
def generate_with_openai(api_key: str, prompt: str, output_path: Path, size: str = "1792x1024") -> None:
    ...
    # Add photojournalistic style to all prompts for natural look
    style_prefix = "Candid photojournalistic photograph, natural daylight, shot with a 35mm lens. Real skin texture, natural expressions, slight asymmetry. Looks like a hospital annual report editorial, not a stock photo. "
    if "infographic" in prompt.lower() or "flat vector" in prompt.lower() or "diagram" in prompt.lower():
        style_prefix = ""  # Don't add photo style to infographics
    ...
    # Post-process: upscale to 1920x1080 and sharpen
...
def main():
    ...
    parser.add_argument("--guide", type=int, required=True, help="Guide number (1-17)")
